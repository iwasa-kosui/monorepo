...
**Architecture:** 概要を「聴衆の課題、登壇者の一次経験、部分適用という主張、40分で扱う内容、二段階の認識変化」の順に再構成する。4つの既存データパターンは維持し、現在の状態だけを保存するCRUDをライブ実装の対象、残り3パターンを判断軸の比較対象とする。

**Tech Stack:** Markdown、dprint、ripgrep、GitHub CLI

## Global Constraints

- ストーリー上の正本はコミット `ffebc8f7a4bfd1ed2fad561dcf5c0e050f073b7a` の `talks/2026/yapc-tokyo-2026/event-sourcing-auditability.md` とする。
- タイトル「もう他人ごとじゃないイベントソーシング: 社会を支えるプロダクトにあとから監査性を縫い込む」は変更しない。
- データについて「現在値」と書かず、「現在の状態」と書く。
- 「社会の礎になるようなプロダクトに関わりたい」を登壇者自身の動機として書かない。
- 元原稿にない感情、迷い、転機、発言、経験を補わない。
...
私もかつては、イベントソーシングを、最初から強い思想で設計されたシステムだけが採用する複雑なものだと考えていました。見方が変わったのは、医療SaaSの共通基盤で、現在の状態しか残っていないデータに向き合ったときです。ユーザー、施設、端末、ライセンスについて、現在の状態は分かる。しかし、そこへ至る変更を追い、問い合わせや障害対応で説明することは難しい。この経験から、イベントを残すことは壮大なアーキテクチャのためだけではなく、プロダクトの信頼を守るための実践だと考えるようになりました。
...
```bash
awk '/^## 概要/{capture=1;next}/^## ハンズオン構成/{capture=0}capture' talks/2026/yapc-tokyo-2026/event-sourcing-auditability.md | tr -d '\n' | wc -m
```

Expected: `948`。900〜1,100字の範囲内。
...
- 「既存プロジェクトには非現実的」から「部分的かつ意義のある活用ができる」への認識変化が伝わるか
