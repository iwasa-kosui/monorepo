...
Embedded SRE不在でも<br/>開発チームが設計を"自分ごと"として運用し続けることで<br/>信頼性は向上できる

</MessageBox>

<!--
まず、本日お伝えしたいキーメッセージを先にお話しします。
「Embedded SREがいないから信頼性向上は難しい」——そう思っていませんか？
私たちの経験では、開発チーム自身が設計を「自分ごと」として運用し続けることで、信頼性は確実に向上できました。
この後、その具体的な方法論と教訓をお伝えしていきます。
-->
...
<!--
本日はこの5つの構成でお話しします。
まず背景として私たちのチームとプロダクトを紹介し、次に直面した課題を説明します。
そして、課題に対する2つの技術レイヤー——インフラとアプリケーション——と、それらを育てるプロセスについての方法論、その結果と教訓、最後にまとめをお伝えします。
-->
...
<!--
簡単に自己紹介させてください。
私は株式会社カケハシで認証権限基盤チームのテックリードをしています。2022年にカケハシに入社し、前職のDeNAではWebプラットフォームの開発に携わっていました。
普段はTypeScript、AWS、PostgreSQL、Terraformなどを使って開発しています。TSKaigi 2024や関数型まつり 2025など、バックエンドTypeScriptや認証基盤に関する登壇もしています。
-->
...
<!--
カケハシについてもう少し紹介させてください。
カケハシは「日本の医療体験を、しなやかに。」をミッションに掲げ、調剤薬局向けのDXプロダクト群を開発・運用しています。
代表的なプロダクトであるMusubiは次世代型の電子薬歴で、そのほかにも経営分析のMusubi Insight、患者向けアプリのPocket Musubi、AI在庫管理など、複数のプロダクトを展開しています。
これらすべてのプロダクトが患者情報を扱うため、高い信頼性とセキュリティが求められます。
-->
...
4つ以上存在するカケハシのプロダクト群が **医療システム** として要求される品質を満たすために  
共通する **サブドメイン** を高品質に解決すること
...
- **3省2ガイドライン**（厚労省・経産省・総務省）への準拠が求められる
- すべてのプロダクトが**患者情報**を扱うため、高い信頼性とセキュリティが必要
...
<!--
私たちのチームについて紹介します。
認証権限基盤チームは、認証ポータル、ディレクトリサービス、アセットサービスという3つの基盤システムを担当しています。
これらは社内の複数プロダクトから利用される共通基盤であり、カケハシのすべてのプロダクトが患者情報を扱うため、高い信頼性とセキュリティが求められます。
特に、厚生労働省・経済産業省・総務省が定めた医療情報の安全管理に関するガイドライン、いわゆる3省2ガイドラインへの準拠が必要です。
-->
...
2025年4月時点では非常に小規模なチーム。Embedded SREを迎える余裕はなく、**開発チーム自身が信頼性に責任を持つ**道を選択。

- コンテキストが集約され、迅速な改善が可能
- 専門的なSRE知見は自ら習得する必要あり
...
> [!NOTE]
> **開発チーム自身が信頼性に主体的に取り組む** — 制約を強みに変える選択

<!--
私たちのチームは小規模で、Embedded SREを迎える余裕はありませんでした。
しかし、コンテキストが集約されるという強みを活かし、開発チーム自身が設計・実装・運用を一貫して担うことを選びました。
医療SaaSでは、3省2ガイドライン準拠、99.9%以上の可用性、完全なトレーサビリティ、セキュアなマルチテナント分離など、非常に高い品質要求が課されています。
薬局は24時間365日稼働しており、「3ヶ月前のこのユーザーの権限は何だったか」を説明できる必要もあります。
これらの要求に、開発チーム自身が責任を持って応えていく。それが私たちの選択でした。
-->
...
このように、プロダクト群はOIDCで認証基盤に接続し、認証基盤はディレクトリサービスを参照します。
-->
...
<CardGrid :cols="2">
  <Card
    title="1. インフラレイヤ"
    description="テナント分離とデータ連携の基盤"
  />
  <Card
    title="2. アプリケーションレイヤ"
    description="整合性とトレーサビリティの実現"
  />
</CardGrid>

<div class="mt-4">
  <Card title="継続的改善プロセス" description="2つのレイヤーを維持・進化させる仕組み（ADR・ポストモーテム）" />
</div>

> [!NOTE]
> 技術レイヤーで**選択肢を理解し、トレードオフを受け入れて選ぶ**。プロセスで**その選択を維持・改善し続ける**
...
| 領域 | 直面した課題 | 我々の選択 |
|:--|:--|:--|
| インフラレイヤ | テナント分離がアプリ実装に依存 / 基盤障害が全体に波及 | RLSによるDB保証 / データ基盤経由をデフォルトに |
| アプリケーションレイヤ | 過去の状態を説明できない / 整合性と独立性のトレードオフ | ドメインイベントの永続化 / サービスベースアーキテクチャ |
| 継続的改善 | 設計判断の根拠が散逸 / 障害が改善に繋がらない | ADRによる記録 / ポストモーテムからの改善 |

> [!IMPORTANT]
> 選択して終わりではない — **運用しながら育て続ける**ことが重要

<!--
各レイヤーでの具体的な選択を一覧でお見せします。
インフラレイヤでは、「テナント分離がアプリ実装に依存する」という課題にRLSで対応し、「基盤障害が全体に波及する」という課題にはデータ基盤経由をデフォルトとしました。
アプリケーションレイヤでは、「過去の状態を説明できない」という課題にドメインイベントの永続化を選び、「整合性と独立性のトレードオフ」にサービスベースアーキテクチャを選びました。
継続的改善プロセスでは、「設計判断の根拠が散逸する」という課題にADRで対応し、「障害が改善に繋がらない」という課題にポストモーテムからの改善で対応しました。
重要なのは、これらは「選んで終わり」ではないということです。
運用しながら課題を見つけ、設計を育て続ける。それが「責任を果たす」ということです。
では、最初にインフラレイヤから詳しく説明していきます。
-->
...
<!--
インフラレイヤの最初のトピック、RLS——Row Level Securityによるマルチテナント分離について説明します。
...
セキュリティは一度設定して終わりではありません。継続的な監視と改善が不可欠です。次に、データ連携パターンについて説明します。
-->
...
| **DynamoDB + Streams** | ◎ | ◎ | ○ | 高可用性が必要なセッション管理 |
...
<CardGrid :cols="2">
  ...
  <OptionCard title="高可用性が必要: DynamoDB">
    <p>認証基盤のセッション管理に採用<br>RDSより高い可用性が必要なため<br>Streamsでイベント連携も可能</p>
  </OptionCard>
</CardGrid>

> [!NOTE]
> **パターンの使い分け:** 基本はデータ基盤経由、セッション管理など高可用性が必要な場合はDynamoDB

<!--
インフラレイヤの2つ目、データ連携パターンの選択について説明します。
...
私たちの選択は、「デフォルトはデータ基盤経由」です。基盤に障害が発生しても、プロダクトは最後に同期されたデータで動作を継続できます。
ただし、認証基盤のセッション管理にはDynamoDB + Streamsを採用しています。RDSより高い可用性が求められるユースケースでは、DynamoDBの特性を活かしています。
-->
...
> ドメインイベントはドメインモデルの正式な構成要素であり、ドメイン内で起こった何かを表現するものである
>
> — Eric Evans
...
イベント駆動アーキテクチャやCQRSの文脈でよく語られるが<br/>
**監査対応や障害調査にも有効**
...
<!--
アプリケーションレイヤの最初のトピック、ドメインイベントについて説明します。
...
<div class="p-5 bg-red-50 rounded-lg mb-4">
  <h3 class="font-bold text-lg">課題: すでに稼働中のシステムにドメインイベントを導入したい</h3>
</div>
...
<div class="mt-4 p-4 bg-brand-50 rounded-lg">
  ...
  <ul class="text-sm space-y-1">
    ...
    <li>データ基盤経由で<span class="font-bold">一貫性を保証</span>しながら変換</li>
...
flowchart TD
  ...
  subgraph "イベント生成"
    Converter[スナップショット<br>→イベント変換]
    EventStore[(イベントストア)]
  end

  OldDB -->|1. CDC| CDC
  CDC -->|2. 蓄積| DeltaLake
  DeltaLake -->|3. 読み取り| Converter
  Converter -->|4. 書き込み| EventStore
```
...
<InsightCard title="Step 1: 並行運用" variant="positive">
  <p>既存DB + イベントストアを並行稼働<br>読み取りは既存、書き込みは両方へ</p>
</InsightCard>

<InsightCard title="Step 2: 読み取り切り替え" variant="positive">
  <p>読み取りをイベントストア経由に<br>不整合があれば既存DBにフォールバック</p>
</InsightCard>

<InsightCard title="Step 3: 完全移行" variant="positive">
  <p>既存DBへの書き込みを停止<br>イベントストアが唯一の真実源に</p>
</InsightCard>
...
<!--
スナップショットからイベントを生成するアーキテクチャを説明します。
左側のフローを見てください。既存DBからCDCでデータを抽出し、Delta Lakeに蓄積します。そこからスナップショットを読み取り、イベントに変換してイベントストアに書き込みます。
右側は導入ステップです。
Step 1は並行運用。既存DBとイベントストアを並行稼働させます。読み取りは既存DB、書き込みは両方へ。
Step 2は読み取り切り替え。読み取りをイベントストア経由に変更します。不整合があれば既存DBにフォールバックできるようにしておきます。
Step 3は完全移行。既存DBへの書き込みを停止し、イベントストアが唯一の真実源になります。
各ステップで元に戻せる設計にすることで、障害時のダメージを局所化できます。
-->
...
<InsightCard title="依存関係の順序制御" variant="negative">
  <p>ユーザー→組織→権限の順序で生成必要<br><span class="highlight" style="color: var(--slidev-code-error)">→ 依存グラフを事前に整理</span></p>
</InsightCard>

<InsightCard title="過去データの「誰が」問題">
...
<InsightCard title="データ基盤が鍵" variant="positive">
  <p>Delta Lakeのタイムトラベルで<br>特定時点のスナップショットを再現可能</p>
</InsightCard>

<InsightCard title="ロールバック戦略" variant="positive">
  <p>各ステップで元に戻せる設計<br>障害時のダメージを局所化</p>
</InsightCard>
...
> [!IMPORTANT]
> **新規システムなら最初から組み込む** — 既存システムへの後付けは工数がかかる

<!--
段階的導入で直面した課題と得られた知見をお話しします。
まず課題として、依存関係の順序制御がありました。ユーザー、組織、権限という順序でイベントを生成する必要があり、依存グラフを事前に整理しておくことが重要でした。
また、過去データには「誰が操作したか」という情報がありません。これは「システム移行」として記録することで対応しました。
得られた知見としては、インフラレイヤで説明したDelta Lakeのタイムトラベル機能が、このような移行でも非常に役立ちます。特定時点のスナップショットを再現できるため、一貫性のあるイベント生成が可能です。
また、各ステップで元に戻せるロールバック戦略を用意しておくことで、障害時のダメージを局所化できます。
新規システムであれば最初からドメインイベントを組み込むことをお勧めします。既存システムへの後付けは工数がかかります。
-->
...
<InsightCard title="イベントの粒度">
  <p>細かすぎると追跡が困難<br><span class="highlight">→ ビジネス単位でイベントを設計</span></p>
</InsightCard>
...
また、イベントの粒度も重要です。細かすぎると追跡が困難になります。ビジネス単位でイベントを設計することをお勧めします。
...
<!--
最後に、技術レイヤーを維持・改善するプロセスについてお話しします。
...
設計を「自分ごと」として運用し続ける。これが私たちの信頼性向上の核心です。
-->
