...
本日は30分で、その具体的な方法論と、1年以上の運用を通じて得た教訓をお伝えします。
-->
...
この30分で、なぜそう言えるのか、具体的にどうやったのかをお話しします。最後にもう一度このメッセージに戻ってきますので、覚えておいてください。
-->
...
<!--
データ基盤連携の具体的な実装です。左側のアーキテクチャ図を見てください。基盤のDBからCDCでデータを抽出し、Delta Lake形式でS3に保存します。プロダクト側はこのDelta Lakeからデータを読み取ります。
...
マイクロサービスとイベント駆動は、誤った分割のリスクと分散トランザクションの複雑さから不採用としました。モジュラモノリスは、認証基盤だけをデプロイするといった細かいデプロイの制御が難しく不採用です。
