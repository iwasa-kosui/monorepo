...
<!--
スナップショットからイベントを生成するステップを詳しく見ます。データ基盤には2つのDelta Lakeがあります。1つは状態テーブルのCDCログを蓄積するもの、もう1つは生成されたドメインイベントを蓄積するものです。
まず初期フェーズでデータ基盤へのCDC蓄積を開始します。次に差分検出フェーズで、CDCログのスナップショット間の差分を検出し、既存イベントを生成します。この既存イベントはDBを経由せず、直接Delta Lakeのドメインイベントテーブルに書き込みます。
最後に新規書き込み対応として、アプリケーション側で新規の書き込みを即座にイベント化し、DBのイベントテーブルに書き込みます。イベントテーブルの内容はCDCでドメインイベント用のDelta Lakeに蓄積されます。
...
- 「どのサービスもDBを直接触れるって、結合度が高くなりそう...」
- 「スキーマ変更したら全サービスに影響しない？」
- 「ただの分散モノリスになりそう」
- 「それはモジュラモノリスの各モジュールを独立デプロイしてるだけでは？」
- 「様々な書籍にも『DB共有は避けろ』って書いてあったような...」
...
<!--
ここで「ちょっと待って」と思った方、いらっしゃると思います。「共有DBってアンチパターンじゃないの？」と。
その懸念はもっともです。マイクロサービスの文脈では「各サービスは自分専用のDBを持つべき」と言われますし、共有DBは密結合の原因になると警告されています。
私たちも最初は同じ懸念を持っていました。しかし、サービスベースアーキテクチャは「共有DBアンチパターン」とは明確に異なります。次のスライドでその違いを説明します。
-->
