...
二つ目は、SREイネイブラーの方。開発チームへの知見移転を進める際に、「Enablingのゴール」として何を目指すべきか、一つの具体例を示せればと思います。
...
<div class="mt-4 p-4 bg-brand-50 rounded-lg">
  <p class="text-sm">
    <span class="font-bold">ミッション:</span> プロダクトチームが3省2ガイドラインを個別に解釈する必要をなくし、本来のビジネスロジックに集中できるようにする
  </p>
</div>
...
<InsightCard
  title="共通の目的"
  variant="positive">
  プロダクトチームが本来のビジネスロジックに集中できるようにする
</InsightCard>

<!--
こちらが私たちの基盤システムの全体像です。
認証ポータルは、MFAやSSOなどセキュアな認証をOIDC連携で提供します。各プロダクトは認証ポータルと連携するだけで、複雑な認証ロジックを実装する必要がなくなります。
ディレクトリサービスは、組織・薬局・ユーザー情報を管理し、データ基盤経由で各プロダクトに提供します。なぜAPI連携ではなくデータ基盤経由なのかは、後ほど詳しく説明します。
アセットサービスは、ライセンスと端末の管理をAPI連携で提供します。こちらはリアルタイム性が必要なため、API連携を選択しています。
これらの共通の目的は、各プロダクトチームが3省2ガイドラインを個別に解釈する負担をなくし、本来のビジネスロジックに集中できるようにすることです。
...
<CardGrid :cols="1">
  <InsightCard title="要件: 認証時にユーザーや組織の情報を参照する必要がある" variant="negative">
...
<CardGrid :cols="2">
  <Card
    title="障害対応がSRE依存になる"
    description="原因特定に2〜3時間。「誰か調べて」が常態化"
  />
  <Card
    title="影響範囲を制御できない"
    description="1つの障害が複数チームを巻き込む。責任の所在が曖昧に"
  />
  <Card
    title="「怖くて触れない」コードが増える"
    description="変更のたびに予期しない影響。デプロイが祈りになる"
  />
  <Card
    title="セキュリティが「人の注意力」頼み"
    description="WHERE句の書き忘れで漏洩。レビューで防げる保証はない"
  />
</CardGrid>

> [!CAUTION]
> これらの問題があると、開発チームは**信頼性を「自分ごと」として担えない**

<!--
具体的な方法論に入る前に、私たちが解決したかった問題を整理します。
信頼性を脅かす問題として、4つを特定しました。いずれも「開発チームが自律的に信頼性を担う」ことを阻む問題です。
1つ目、障害対応がSRE依存になる。原因特定に2〜3時間かかり、「誰か調べて」が常態化する。開発チームが自走できません。
2つ目、影響範囲を制御できない。1つの障害が複数チームを巻き込み、責任の所在が曖昧になる。「自分たちの責任」と言われても、範囲が見えないのです。
3つ目、「怖くて触れない」コードが増える。変更のたびに予期しない影響があり、デプロイが祈りになる。これではYou Build It, You Run Itは成立しません。
4つ目、セキュリティが「人の注意力」頼み。WHERE句を忘れたら漏洩する。レビューで防げる保証はありません。
SREイネイブラーの皆さん、これらの問題があるチームに「信頼性は自分たちの責任」と言っても、どうすればいいかわからない。Enablingの前提として、まずこれらを解消する必要がある。私たちは4つの方法論で対応しました。
-->
...
<CardGrid :cols="2">
  <MethodCard :number="1" title="ドメインイベントの永続化" description="「何が起きたか」を完全に記録し、追跡可能に" />
  <MethodCard :number="2" title="データ連携パターンの選択" description="基盤障害がプロダクト全体に波及しない設計"/>
  <MethodCard :number="3" title="サービスベースアーキテクチャ" description="強い整合性を保ちながら独立したデプロイを実現" />
  <MethodCard :number="4" title="RLSによるマルチテナント分離" description="DBレベルで強制的にテナントを分離" />
</CardGrid>
...
<!--
それでは、4つの方法論の全体像をお見せします。
1つ目はドメインイベントの永続化。「何が起きたかわからない」という問題に対して、完全な記録で追跡可能にします。
2つ目はデータ連携パターンの選択。「一箇所の障害が全体に波及する」問題に対して、影響範囲を限定する設計をします。
3つ目はサービスベースアーキテクチャ。「変更が難しい」問題に対して、整合性を保ちながら独立したデプロイを実現します。
4つ目はRLSによるマルチテナント分離。「データが漏れる」問題に対して、DBレベルで強制的に分離します。
重要なポイントは、これらは特別なツールや大規模な組織変更を必要としないということです。既存の技術を組み合わせて、段階的に導入できます。
では、1つ目のドメインイベントの永続化から詳しく説明していきます。
-->
...
まず、後付け導入の難しさ。既存システムにドメインイベントを導入する際、既存データからイベントを生成する必要がありました。しかし、依存関係を考慮した順序制御が必要で、想定以上に工数がかかりました。これから導入を検討される方は、新規開発時に組み込むことを強くお勧めします。
...
モノリスは変更容易性が低く、一度に全体を更新する必要があります。チームが小さいうちは良いですが、スケールしにくい。
...
SREイネイブラーの観点からは、これが「開発チームが自分事として信頼性を担う状態」——Enablingの目標状態の一つの形だと考えています。
-->
...
<!--
SREイネイブラーの方に向けて、Enablingの観点からまとめます。
...
本セッションの事例は、Enablingのゴールの一つの形だと考えています。開発チームへの知見移転を進める際の参考になれば幸いです。
