...
| R-H2 | 方法論 | ドメインイベント、データ連携パターン、サービスベースアーキテクチャ、RLS |
...
| 方法論 | ドメインイベントの永続化、データ連携パターンの選択基準（API/イベント/データ基盤）、サービスベースアーキテクチャ、RLSによるマルチテナント分離 |
...
| カテゴリ | 理由 |
|----------|------|
| 認証基盤の統一（OIDC, MFA, 監査ログ） | 特定ドメインの話になり汎用性が低い |
...
| **方法論** | 12分 | ドメインイベント、データ連携パターン、サービスベースアーキテクチャ、RLS |
...
4. **RLSの仕組み図**: 行レベルセキュリティによるマルチテナント分離
5. **「責任を果たす」サイクル図**: 設計→運用→監視→改善のサイクル
...
| 2026/01/17 | 2.1 | RLSをスコープ内に追加（マルチテナント分離は汎用的なトピックのため） |
