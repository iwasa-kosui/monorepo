...
<!--
1つ目の落とし穴、構造的部分型です。User と Product は名前が違いますが、id と name という同じ構造を持っており、sortByUserId に Product を渡してもエラーになりません。
なぜこうなるか。Java ではクラスローダーが型のアイデンティティを管理しますが、JavaScript にはそれが存在しません。あるプロパティを持つオブジェクトは、class で作ろうがオブジェクトリテラルで作ろうが同じように使える。これが JavaScript の柔軟性で、TypeScript はこの柔軟性を壊さないために構造的部分型を採用しました。互換性のための必然的な選択です。
-->
...
<!--
落とし穴1への対策は Branded Type です。ユニークな symbol プロパティと文字列タグで型にブランドを付与し、構造が同じでも異なる型として扱えるようにします。Zod の brand を使えば外部入力のバリデーションとブランド付与を同時にできます。
...
<!--
2つ目の落とし穴、this バインディングです。メソッドをコールバックとして渡すと this が undefined になり TypeError が出ます。
...
# 対策2: 「this を持たない関数」に逃がす

```typescript
// 対策2-a: class 内で対処 — アロー関数フィールドで lexical this
class Uploader {
  upload = (fileName: string) => { console.log(`${fileName} → ${this.storage}`); };
}

// 対策2-b: class から離脱 — そもそも this を作らない
type Uploader = Readonly<{ storage: string }>;
const upload = (u: Uploader) => (fileName: string) =>
  console.log(`${fileName} → ${u.storage}`);
```

- **2-a**: lexical this で切り離しを防ぐ。ただし decorator / AOP は効かない
- **2-b**: this を持たない関数なら、そもそも切り離す対象がない

<!--
対策は大きく2つです。
2-a は class を使い続ける場合の基本パターン、アロー関数フィールド。lexical this で this を切り離さないようにします。ただし prototype に載らずインスタンスごとにコピーされ、decorator や AOP の対象外になる注意点があります。
2-b は class から離れて関数として書くパターン。this を持たない関数なら、そもそも切り離す対象がありません。落とし穴2は構造的に発生しなくなります。後半の「クラスがない世界」で改めて掘ります。
-->

---

# 落とし穴3: private 修飾子と型消去
...
<!--
3つ目の落とし穴、private 修飾子です。TS の private は any キャストやブラケット記法で簡単に回避でき、トランスパイル後の JavaScript には何の痕跡も残りません。
...
<!--
落とし穴3への対策は ES の # private フィールドです。TS 3.8 からサポートされています。# private は構文レベルで実行時アクセスを禁じるため、any キャストでも Reflect.ownKeys でも到達できません。
...
<!--
4つ目の落とし穴、instanceof です。interface に対して instanceof を使おうとするとコンパイルエラーになります。interface はトランスパイルで消えるため、実行時には存在しません。
...
# 対策4: Discriminated Union でタグを値に置く
...
<!--
落とし穴4への対策は Discriminated Union です。kind というタグフィールドを持たせ、switch で型を判別します。
...
ここで再び、対策はすべて「値」で出来ている、という構図が見えてきます。
-->
...
new Uploader(mockStorage).upload("x");
```
...
```typescript
type Storage = { write(s: string): Promise<void> };
const upload = (storage: Storage) =>
  (file: string) => storage.write(file);

// テスト: シグニチャを満たす値で済む
await upload({ write: async () => {} })("x");
```
...
- 関数の **(入力型 → 出力型)** を読めば、classloader に頼らず振る舞いが分かる
- テストはシグニチャを満たす値で十分。コンストラクタもモックライブラリも要らない
...
<!--
値の世界の利点は、契約が関数のシグニチャに集約されることです。入力型と出力型を読めば、その関数の振る舞いが理解できる。classloader にもリフレクションにも依存しません。
左側の class 版では、テストにモックライブラリを使って Storage の足場を組む必要があります。右側の値版では、シグニチャ Storage を満たすダミーオブジェクトをそのまま渡すだけ。コンストラクタを通す手間も、モックの設定もありません。
