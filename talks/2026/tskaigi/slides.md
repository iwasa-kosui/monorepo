...
<!--
本日は大きく4つの章で進めます。まず歴史編で TypeScript の class がなぜ今の形になったかを追います。ここで Q3 の答えが出ます。
次に「特性と対策」編では、TypeScript の class を生み出している3つの特性それぞれについて、それが招く落とし穴と対策をひと続きで見ていきます。Q1 と Q2 の答えはここで出ます。
3部目「クラスがない世界」では視点を切り替えて、「そもそも class を使わない世界」が TypeScript ではどう成り立つのかを見ます。
4部目「class を使う」では逆に、Custom Error や Symbol.dispose のように class が自然に活きる場面を扱います。
最後にまとめとして、立場ごとの付き合い方を整理します。
-->
...
a は class を使い続ける場合の基本パターン、アロー関数フィールドです。lexical this で固定することで、this を切り離されないようにします。ただしメソッドが prototype ではなくインスタンスごとに作られるので、メモリ消費が増える点と、prototype 前提のフレームワーク機能（NestJS の DI など）と相性が悪い点には注意です。
...
TypeScript には実行時に型のアイデンティティを問い合わせる手段がなく、私たちはこれを「不便」と捉えがちです。
...
<!--
値の世界では、契約が関数のシグニチャに集約されます。入力型と出力型を読めば、何を渡せば何が返るかが決まります。
これは「実行時の型情報がない」ことの直接的な結果です。型情報が実行時に消えるからこそ、契約はすべて静的にシグニチャに書き切られています。一見制約のように見えますが、IDE 補完・型推論・grep だけで振る舞いを追える、という強力な静的読解性を実現します。
-->
...
<!--
Custom Error は class が自然に活きる代表例です。Error を継承することで stack trace や cause チェーンといった Error の機能をそのまま引き継げます。
catch 節での instanceof による判定は JavaScript の慣用句で、これは Discriminated Union では代替しづらい場面です。tag フィールドを持たせる方法もありますが、try/catch との統合を考えると instanceof の方が自然です。
-->
...
<!--
リソースのライフサイクル管理も class が活きる場面です。Symbol.dispose プロトコルを実装することで、TypeScript 5.2 以降の using declaration と組み合わせて、ブロック終了時に自動的に dispose が呼ばれるようにできます。
DB 接続、WebSocket、ファイルハンドルなど、open と close のペアで使うリソースに最適です。
-->
