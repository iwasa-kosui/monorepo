...
```mermaid {scale: 0.8}
graph LR
  A[構造的部分型] --> P1[落とし穴1: 異なるclassが同じ型]
  B[型消去] --> P3[落とし穴3: privateが実行時に効かない]
  B --> P4[落とし穴4: instanceofが使えない]
  A -.-> P2[落とし穴2: thisの消失]
  B -.-> P2
...
- **構造的部分型** → 落とし穴1（構造的部分型）
- **型消去** → 落とし穴3（private）、落とし穴4（instanceof）
- **両方が関与** → 落とし穴2（this）：prototypeベースOOP + 型では防げない

<!--
4つの落とし穴を根本原因にマッピングしたのがこの図です。構造的部分型が原因なのが落とし穴1。型消去が原因なのが落とし穴3と4。落とし穴2のthisバインディングは、prototypeベースOOPの仕様であり、型だけでは防げないという意味で両方が関与しています。
根本原因を理解したところで、次はこれらの落とし穴にどう対処するかを見ていきましょう。
-->
