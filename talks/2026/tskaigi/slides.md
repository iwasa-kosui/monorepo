...
<!--
1つ目の特性、構造的部分型です。型の互換性が、クラスの名前ではなく、プロパティの構造で決まる仕組みです。Java / C# / Rust のような言語では classloader 等で型のアイデンティティを実行時に保持しているため、同じ構造でも別名のクラスは区別されます。一方、JavaScript には型のアイデンティティが存在しません。class でもオブジェクトリテラルでも同じように扱えるのが JS の前提で、TypeScript はそれを維持する設計を選びました。次に、この特性が具体的にどんな落とし穴になるのかを見ます。
-->
