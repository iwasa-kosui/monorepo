...
item instanceof Drawable;  // Uncaught ReferenceError: Drawable is not defined

class Account { private balance = 10000; }
(new Account() as any).balance;  // 10000
```

- `interface` / `type` は instanceof の対象にできない  
  トランスパイル時に型消去される
- `private` / `protected` はトランスパイル時のみ検証される  
  実行時には検証されない
...
`kind` を**値**として持ち switch + `never` で網羅性検証
