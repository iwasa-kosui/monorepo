...
- `interface` / `type` は実行時に存在しない → instanceof の対象にできない
- トランスパイル後の JavaScript には `Drawable` という識別子が残らないため、参照しようとすると ReferenceError

<!--
型消去が引き起こす落とし穴の代表例が instanceof です。interface はトランスパイル時に消えるので、JavaScript 出力には Drawable という識別子そのものが存在しません。instanceof を使おうとすると、TS のコンパイル時には「only refers to a type」エラー、もし強引にトランスパイルさせれば実行時に ReferenceError になります。
これは「型として宣言したものは実行時には存在しない」という型消去の直接的な帰結です。
-->
