...
<!--
本日は大きく2部構成です。まず歴史篇でTypeScriptのclassがなぜ今の形になったかを追います。ここでQ3の答えが出ます。
...
- ES3 策定後、TC39 は class 宣言・強い型付け・namespace を含む **ES4** の策定に着手
- 後方互換性の破壊と仕様の複雑化を懸念する勢力（Microsoft 等）が反対
- 標準化プロセスが分裂し、まとまらないまま放置に

### 結果

- 2008年にES4は**正式に廃棄**
- 代わりに**Harmony**プロセスが発足、ES5は控えめな改善にとどめ class は次世代へ
- 同時期に **CoffeeScript** など「class を持つ AltJS」が広がる

<!--
classの歴史は、実はES4の挫折から始まります。ES3の後、TC39はclass宣言や強い型付けを含む野心的なES4を策定しようとしました。しかし後方互換性の破壊と仕様の複雑化への懸念から、Microsoftを中心に複数のメンバーが反対し、2008年に廃棄されました。ここでclass構文は一度お蔵入りになり、Harmonyプロセスで仕切り直しとなります。
class構文を持たないJavaScriptに対する人々の渇望は、その後CoffeeScriptに代表されるAltJSの隆盛として表面化することになります。
-->
...
<InsightCard title="全員が合意できる最小限の設計">
  <p>Allen Wirfs-Brock らが中心的に推進</p>
...
しかし2022年にStage 3に到達した新仕様はlegacyと互換性がありません。APIが全く異なります。TypeScript 5.0で新仕様がサポートされましたが、--experimentalDecorators は後方互換のため存続しており、フラグなしでは新仕様が適用されます。
2026年の今、採用判断は分かれます。Angular/NestJS/TypeORM はリフレクトメタデータに強く依存しており、Stage 3 への移行はまだ進んでいません。新規にライブラリを書くなら Stage 3 一択ですが、既存のフレームワーク依存があるアプリは legacy 継続が現実解です。
-->
...
| 年 | 概要 | 詳細 |
|---|---|---|
| 2008 | **ES4 廃棄** | class 標準化が見送られる |
| 2009 | **CoffeeScript 登場** | class 構文を持つ AltJS が広がる |
| 2012 | **TypeScript 0.8 公開** | ES2015 より3年先に class を実装 |
| 2015 | **ES2015** | maximally minimal な class 構文が標準化 |
| 2022 | **ES2022** | private / static fields が標準化 |
| 2023 | **TypeScript 5.0** | Stage 3 decorators をサポート |

<!--
全体像を俯瞰しましょう。ES4が廃棄された2008年から TypeScript 5.0 の2023年まで、約15年の歴史です。
2008年のES4廃棄で class 標準化が見送られた後、2009年に CoffeeScript が class 構文を持って登場し、人々の渇望に応えました。その流れの中で2012年に TypeScript がES2015より3年早く class を実装します。その後ECMAScriptが追いかけ、2015年・2022年と段階的に標準化が進み、最新の Stage 3 decorators が2023年に TypeScript に到着するまで、TypeScript と ECMAScript の追いかけっこが続いています。
-->
...
これから紹介する**4つの落とし穴**は  
この3つの根本原因の組み合わせで起きている

</div>

<div class="mt-4 text-sm opacity-70 text-center">

前2つは TypeScript の型システム側の選択、最後の1つは JavaScript ランタイム側の仕様
...
前2つはTypeScript側の選択、最後の1つはJavaScript側の事情、と性質が違う点も重要です。これから紹介する4つの落とし穴は、突き詰めるとこの3つの組み合わせで起きています。
-->
...
const ProductId = (s: string) => s as ProductId;
```

文字列タグでブランドを付与 → 構造が同じでも**型レベルで区別**できる

<div class="mt-4 text-sm opacity-80">

`brand` を `unique symbol` にすることで、JS としてのランタイムコストはゼロ

</div>

<!--
落とし穴1への対策です。Branded Typesは、ユニークなシンボルプロパティと文字列タグで型にブランドを付与する手法です。構造が同じでも異なる型として扱えるようになります。
brand を declare const brand: unique symbol にする点が重要で、これがないと型レベルの分岐が効きません。Effect-TSやio-tsなど主要な型ライブラリでも採用されている近年の標準パターンです。
ランタイムコストはゼロ、つまり brand プロパティはコンパイル時の幻影なので、実行時のオブジェクトには何も付け足されません。
-->

---

# 対策1: class のフィールドにも使える
...
sortByUserId([new User(UserId("1"), "田中")]);       // OK
sortByUserId([new Product(ProductId("2"), "商品A")]); // 型エラー!
```

NestJS / TypeORM の Entity でも、**コンストラクタの型を Branded Type にするだけ**

<!--
class を使い続けるコードでも、コンストラクタやフィールドの型を Branded Type にすれば、取り違えは型レベルで弾けます。NestJS や TypeORM の Entity でも同じです。書き換え自体は小さく、ID系から段階導入できます。
-->

---

# 対策1: Zod でバリデーションと一体化
...
const user = UserId.parse(req.params.id); // バリデーション + ブランド付与
```

外部入力の **バリデーション** と **ブランド付与** を同時に実現できる

<!--
Zod の brand を使えば、外部入力のバリデーションとブランド付与を同時に実現できます。Zod v4 では brand のジェネリクスに文字列タグを渡す形が標準です。
外部入力のバリデーションに Zod を使っているプロジェクトなら、brand を足すだけで導入できます。
-->
...
- 歴史篇で見た TypeScript 0.8 の「コンパイル後 ES5」を思い出してほしい
- 当時の `Greeter.prototype.greet = function () { ... }` がそのまま今の class
- ES2015 以降の class は構文糖。メソッドは `Class.prototype` 上の関数オブジェクトであり、`this` の動的束縛は class が隠しているだけ
...
歴史篇で TypeScript 0.8 のコンパイル後の ES5 を見ていただきました。あの prototype の図を覚えていますか。今書いている class の中身は当時のままで、syntax sugar が掛かっているだけです。ES2015のclassはprototypeチェーンのシンタックスシュガーなので、この動的束縛の仕組みがそのまま適用されます。classの見た目がJava風でも、中身はprototypeベースのオブジェクト指向です。
-->

---

# 対策2-a: アロー関数フィールド（class 内で対処）
...
this を**語彙的に**束縛して、変数代入でも this を切り離さない

<div class="mt-4 text-sm opacity-80">

注意: prototype に載らずインスタンスごとにコピーされる。decorator や AOP は効かない

</div>

<!--
落とし穴2への対策の1つ目は、アロー関数フィールドです。class を使い続ける場合はこれが基本パターンになります。アロー関数は宣言時の this を語彙的に束縛するので、変数代入やコールバック渡しでも this が切り離されません。
ただし注意点があります。prototype に載らずインスタンスごとに関数オブジェクトがコピーされること、そしてメソッドではなくフィールドなので decorator や AOP の対象外になることです。NestJS の @UseInterceptors を効かせたいメソッドは prototype メソッドのままにしておく必要があります。
-->

---

# 対策2-b: コンパニオンオブジェクト（class から離脱）
...
データと振る舞いを分離 → **そもそも this が存在しないので、問題が根本から発生しない**

<!--
2つ目はコンパニオンオブジェクトパターンです。class から離れてよい場面ではこれが最も安全です。型とコンパニオンオブジェクトの名前を揃えるのが TypeScript の慣用句で、Effect-TS など関数型寄りのライブラリでもよく見ます。
データと振る舞いを分離しているので、そもそも this が存在しません。落とし穴2は根本的に発生しないのが強みです。
-->
...
`#` プレフィックスのフィールドは **実行時にもアクセス不可** なハードプライバシー

<!--
落とし穴3への対策は、ECMAScriptのシャープprivateフィールドです。TypeScript 3.8 からサポートされています。
シャープprivateは構文レベルで実行時のアクセスを禁じるため、any キャストでも `Reflect.ownKeys` でも到達できません。型レベルの「お願い」だった TS の private とは保護のレイヤーが違います。
-->

---

# 対策3-b: TS `private` と ES `#private` の違い
...
| `structuredClone` | OK | TypeError |
| `JSON.stringify` 出力 | 含まれる | 含まれない |
| Proxy 越しのアクセス | OK | TypeError になりがち |

</div>

外部公開ライブラリは `#private`、内部アプリは `private` で済むことも多い

<!--
両者の違いを表で整理しました。コンパイル時の保護はどちらも持っていますが、それ以外の実行時挙動は全く異なります。
ライブラリや SDK など外部に公開するコードでは、ESのシャープprivateが安全です。一方アプリケーション内部のコードではTSのprivateで十分な場合もあります。
注意点は structuredClone や JSON.stringify との相性です。シャープprivateを持つオブジェクトはそのままだと構造クローンできず、JSON にも出力されません。永続化やプロセス間転送する型では、それ込みで設計する必要があります。
-->
...
ここまで4つの落とし穴と対策を見てきた。**そもそも class を使うかどうかは、多くの場合自分で選べない**
...
| **Web Components** | `class extends HTMLElement`が仕様上必須（Lit などのライブラリで補助） |
...
class を**書く側**は落とし穴を知った上で付き合う。**書かない側**は、フレームワーク選択時にこの表を批評の材料に使う

<!--
4つの落とし穴と対策を一通り見ていただいたところで、一度ズームアウトします。私たちは class を使うかどうか、本当に自分で選べているのでしょうか。多くの場合、フレームワークが決めます。
AngularやNestJSではclassとdecoratorが設計の基盤です。Web Componentsもclass extends HTMLElementが仕様上必須で、Litのようなライブラリが書きやすさを補っています。
一方でReactやVueは関数的アプローチに移行し、HonoやElysiaは最初からclass不使用です。
class を書く側は、ここまでの対策を知った上で付き合うしかありません。class を避けてきた側にとっても、フレームワーク選択や技術選定でこの表を批評の材料として使えます。
-->
...
<CardGrid :cols="3">
  ...
  <SummaryCard :number="2" title="3つの根本原因" description="構造的部分型 / 型消去 / prototypeの動的this" subdescription="個別暗記ではなく原理の理解" />
  <SummaryCard :number="3" title="立場で変わる付き合い方" description="class を書く側はBranded Type と #private で殴る、書かない側はフレームワークの選択を批評する語彙を持つ" subdescription="同じ原理から、立場ごとの戦略が出る" />
</CardGrid>

<!--
持ち帰りポイントを3つにまとめます。
1つ目、classの複雑さは歴史的必然です。ECMAScriptとの追いかけっこの産物であり、設計ミスではありません。
2つ目、構造的部分型・型消去・prototypeの動的thisという3つの根本原因を理解すれば、落とし穴が体系的に見えるようになります。
3つ目、立場で持ち帰り方が変わります。NestJS や Angular で class を書き続ける方は、Branded Type と #private を武器にして付き合う。React や Hono で class を避けてきた方は、なぜ自分の選択が正しかったかの歴史的根拠と、フレームワークが class を要求するときに批評できる語彙を持ち帰ってください。
-->
