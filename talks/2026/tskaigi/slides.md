...
<!--
1つ目の特性、構造的部分型です。型の互換性が、型名や継承関係ではなく、プロパティの構造で決まる仕組みです。Java / C# / Rust のような名前的部分型の言語では、型名と明示的に宣言された継承関係（extends / implements / trait 実装）で互換性が決まるため、同じ構造でも別名のクラスは区別されます。一方、JavaScript には型のアイデンティティが存在しません。class でもオブジェクトリテラルでも同じように扱えるのが JS の前提で、TypeScript はそれを維持する設計を選びました。次に、この特性が具体的にどんな落とし穴になるのかを見ます。
-->
