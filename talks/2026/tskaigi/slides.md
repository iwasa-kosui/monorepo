...
<!--
2つ目の特性、型消去です。TypeScript の型情報はトランスパイル時にすべて削除され、実行時の JavaScript には残りません。「JavaScript に型を追加する」という TypeScript の設計原則の表れで、型は実行時の振る舞いに干渉せず、トランスパイル時にだけ存在します。そのため interface・type・private/protected のような「型として宣言したもの」は、実行時のコードからは参照できません。この特性が、instanceof と private/protected の2つの落とし穴の原因になります。
-->
...
item instanceof Drawable;  // Error: only refers to a type

class Account { private balance = 10000; }
(new Account() as any).balance;  // 10000 — any キャストで素通り
```

- `interface` / `type` は instanceof の対象にできない
- `private` / `protected` は型レベルの「お願い」で実行時にバイパスされる

<!--
型消去が引き起こす落とし穴は 2 つあります。
1 つは interface や type に対する instanceof。これらはトランスパイル時に消えるので instanceof の対象にできず型エラーになります。
もう 1 つは private / protected。型レベルでアクセス制限を表明しますが、トランスパイル後の JavaScript には残りません。any キャストやブラケット記法で簡単に素通りします。
どちらも「型として宣言したものは実行時には存在しない」という型消去の直接的な帰結です。
-->
...
switch (s.kind) {
  ...
  default: { const _: never = s; return _; }  // 網羅性
}
```

`kind` を**値**として持ち、switch + `never` で網羅性チェック

</div>

</div>

値として残るパターンは、最終章「値の世界」に直結する
...
<!--
落とし穴への対策には 2 方向あります。
左は class ベース。Symbol.hasInstance を class の static メソッドとして実装し、instanceof の挙動を中で再定義します。型ガード v is Circle も書けるので narrowing も効きます。ただし IsCircle のような class を新たに作って静的メソッドを生やす必要があり、書く量が多いです。
右は値ベース。kind というタグフィールドを値として持たせ、switch で判別します。default 節の never による網羅性チェックで、新しい variant を追加して case を書き忘れたらコンパイル時に弾けます。
ポイントは、値ベースの kind は実行時にも残る「値」だということです。class を作らなくても、オブジェクトリテラルでも書ける。この「値として残るものに頼る」パターンが、後半「値の世界」で再利用されます。
-->
