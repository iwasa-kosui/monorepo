...
この問題は、特に開発が進んでしばらくしてから顕在化することが多いように感じます。最初は`LogPayload`専用のオブジェクトを`{ id: user.id, role: user.role }`のように明示的に構築していたのが、「`User`はどうせ`id`と`role`を持っているのだから直接渡せばいい」と誰かがスプレット構文でショートカットした瞬間に、`email`が静かにログに混入し始めます。後から`User`に`phoneNumber`フィールドが追加されても、型エラーは出てくれません。
...
<CodePlayground client:visible code={[
  ...
  'safeLog(user);',
  'safeLog({ ...user });',
].join('\n')} lang="typescript" title="RejectSensitiveによる型レベル検出" />
...
].join('\n')} lang="typescript" title="Branded Typeのバイパス" />
...
].join('\n')} lang="typescript" title="Sensitive型によるPII防御" />

このアプローチのミソは、防御がデータ構造自体に組み込まれている点です。Zod stripのような出口での防御は、ログ出力箇所ごとに毎回呼ぶ必要があり、呼び忘れの箇所が増えるほどリスクが高まります。Sensitive型なら入口（ドメインモデル定義）で一度ラップすれば、出口で何もしなくても漏れません。適用箇所がフィールド定義の1箇所に集約されるので、出口N箇所に依存するアプローチより管理しやすいです。
