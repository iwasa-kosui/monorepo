...
余剰プロパティチェックはオブジェクトリテラルの直接代入時にのみ例外的に発動する仕組みなので、変数やスプレッドを経由すれば効きません。スプレッド構文内のインラインオブジェクトリテラルでも余剰プロパティチェックを効かせてほしいという[要望](https://github.com/microsoft/TypeScript/issues/39998)はありますが、2026年3月時点では未対応です。
...
// console.logではutil.inspectが使われ、util.inspect.customが呼ばれる
console.log(user);
// => { id: '1', role: 'admin', email: '[REDACTED]' }
