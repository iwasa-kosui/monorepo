...
async function loadFont() {
  const localFontPath = join(kosuiMeRoot, 'scripts', 'fonts', 'NotoSansJP-Bold.ttf');

  if (existsSync(localFontPath)) {
    console.log('Using local font file...');
    const fontBuffer = readFileSync(localFontPath);
  ...
  const fontsDir = dirname(localFontPath);
  if (!existsSync(fontsDir)) {
    mkdirSync(fontsDir, { recursive: true });
  }
  writeFileSync(localFontPath, Buffer.from(fontArrayBuffer));
...
async function main() {
  console.log('Generating Talk OGP images...');
  ...
  console.log('Talk OGP image generation complete!');
}
