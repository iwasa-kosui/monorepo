...
GAS は WebSocket を扱えないため、**1 分毎の時間トリガーで `conversations.history` をポーリングする方式** に切り替えている。
最大 1 分の遅延が許容できる用途を想定。
...
├── app_manifest.yml            # Slack App マニフェスト (Bot 用)
...
3. **OAuth & Permissions** 画面で「Install to Workspace」を押し、`Bot User OAuth Token` (`xoxb-...`) を控える
...
pnpm exec clasp create --type standalone --title "slack-thread-expander" --rootDir ./dist
# 生成された .clasp.json を ./.clasp.json として保持 (gitignore 済)
```

`.clasp.json` の `rootDir` が `./dist` になっていることを確認すること
(`.clasp.json.example` がテンプレート)。
