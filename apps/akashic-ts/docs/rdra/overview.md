...
goals:
  ...
  - id: "GOAL-005"
    name: "拡張可能な権限モデル"
    description: "基盤はロール＋パーミッション（resource:action形式）の汎用的な仕組みを提供し、具体的なリソース・アクションの定義はOSS利用者に委ねる"
...
contexts:
  ...
  - id: "BIZ-005"
    name: "authorization"
    description: "ロール・パーミッションの定義と、組織単位またはグループに対するユーザーへのロール割当"
...
| ロール | パーミッション（resource:action形式）の集合。具体的な定義はOSS利用者に委ねる |
| パーミッション | resource:action形式のアクセス権。基盤は仕組みのみ提供 |
...
| GOAL-005 | 拡張可能な権限モデル | ユーザー, 組織管理者, プロダクト提供者 |
