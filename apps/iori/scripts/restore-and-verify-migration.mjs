...
export const restoreAndVerifyMigration = async ({ env, signal }) => {
  signal.throwIfAborted();
  const target = await loadExpectedTarget(env.IORI_MIGRATION_EXPECTED_TARGET_PATH);
  const required = (key) => {
    if (!env[key]) throw new Error('Fresh verification configuration is required.');
    return env[key];
  };
  if (
  ...
  const storage = transferStorageFromEnvironment(env, targetIdentity(target), true, signal);
  ...
  const sealed = await readSealedMigrationTarget({
    expectedTarget: target,
  ...
  signal.throwIfAborted();
  return { target, record: summary.preparation.record, ...sealed };
};
