...
import type { RemoteLikeDeletedStore } from '../../../domain/like/like.ts';
import type { IoriD1Db } from '../client.ts';
import { domainEventsTable, likesTable, remoteLikesTable } from '../schema.ts';

export const createD1RemoteLikeDeletedStore = (db: IoriD1Db): RemoteLikeDeletedStore => ({
  store: async (...events) => {
    if (events.length === 0) return RA.ok(undefined);
    const ids = events.map((event) => event.eventPayload.likeId);
    await db.batch([
      db.delete(remoteLikesTable).where(inArray(remoteLikesTable.likeId, ids)),
