...
export const PostForm = ({ id, formId }: Props) => {
  ...
  const handleContentChange = (e: Event) => {
    ...
    if (typeof window !== "undefined" && (window as unknown as { marked?: { parse: (text: string, options?: { async: boolean }) => string } }).marked) {
      const rawHtml = (window as unknown as { marked: { parse: (text: string, options?: { async: boolean }) => string } }).marked.parse(value, { async: false });
