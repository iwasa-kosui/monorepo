  ...
  const run = async (input: Input) =>
    RA.flow(
      ...
      RA.bind("localActorCreated", ({ user, ctx }) =>
        LocalActor.createLocalActor(user, ctx, now)
      ),
      RA.andThrough(({ user }) => {
      ...
      RA.andThrough(({ localActorCreated }) =>
        localActorCreatedStore.store(localActorCreated)
      )
    );
