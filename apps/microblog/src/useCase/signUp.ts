...
import { type UserResolverByUsername, User, UsernameAlreadyTakenError } from '../domain/user/user.ts';
...
const create = ({
  ...
  const errIfUsernameTaken = (existingUser: User | undefined): RA<void, UsernameAlreadyTakenError> => {
    if (existingUser) {
  ...
  const run = async (input: Input) =>
    RA.flow(
      RA.ok(input),
      RA.bind('existingUser', ({ username }) => userResolverByUsername.resolve(username)),
      RA.andThrough(({ existingUser }) => errIfUsernameTaken(existingUser)),
      RA.mapErr(x => x),
      RA.bind('userCreated', ({ username }) => RA.ok(User.createUser(now)({ username }))),
      RA.andThrough(({ userCreated }) => userCreatedStore.store(userCreated)),
      RA.bind('user', ({ userCreated }) => RA.ok(userCreated.aggregateState)),
      RA.bind('localActorCreated', ({ user, ctx }) =>
        RA.ok(LocalActor.createLocalActor(user, ctx, now))
      ),
      RA.andThrough(({ user }) => {
